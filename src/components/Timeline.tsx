    ...
    <div className="relative rounded-[2rem] border border-ink-200 bg-white/80 p-6 shadow-card dark:border-white/10 dark:bg-white/[0.05]">
      ...
      <div className="grid gap-5">
        {events.map((event, index) => (
          <div key={`${event.date}-${index}`} className="relative grid gap-1 pl-10">
            <span className="absolute left-[1.125rem] top-1.5 h-4 w-4 rounded-full border-2 border-white bg-archive-gold shadow-glow dark:border-archive-navy" />
